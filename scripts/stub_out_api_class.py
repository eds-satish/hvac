...
from hvac.api.auth.{{ class_name|lower }} import DEFAULT_MOUNT_POINT
...
from hvac.api.auth import {{ class_name }}
from hvac.api.auth.{{ class_name|lower }} import DEFAULT_MOUNT_POINT
...
.. note::
    Every method under the :py:attr:`Client class's {{ class_name|lower }} attribute<hvac.v1.Client.{{ class_name|lower }}>` includes a `mount_point` parameter that can be used to address the {{ class_name }} auth method under a custom mount path. E.g., If enabling the {{ class_name }} auth method using Vault's CLI commands via `vault auth enable -path=my-{{ class_name|lower }} {{ class_name|lower }}`", the `mount_point` parameter in :py:meth:`hvac.api.auth.{{ class_name }}` methods would be set to "my-{{ class_name|lower }}".
...
.. code:: python
    ...
    {{ class_name|lower }}_auth_path = 'company-{{ class_name|lower }}'
    description = 'Auth method for use by team members in our company's {{ class_name }} organization'

    if '%s/' % {{ class_name|lower }}_auth_path not in vault_client.list_auth_backends():
        print('Enabling the {{ class_name|lower }} auth backend at mount_point: {path}'.format(
            path={{ class_name|lower }}_auth_path,
        ))
        client.enable_auth_backend(
            backend_type='{{ class_name|lower }}',
            description=description,
            mount_point={{ class_name|lower }}_auth_path,
        )
...
:py:meth:`hvac.api.auth.{{ class_name }}.{{ name }}`
